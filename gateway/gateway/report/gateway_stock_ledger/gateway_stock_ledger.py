...
def execute(filters=None):
	...
	if opening_row:
		opening_row["in_rolls"] = 0
		opening_row["out_rolls"] = 0
		opening_row["bal_qty_rolls"] = 0
	...
	for sle in sl_entries:
		...
		# --- Roll calculations ---
		in_rolls = out_rolls = 0

		if sle.serial_and_batch_bundle:
			bundle_rolls = frappe.db.get_all(
				"Serial and Batch Entry",
				filters={"parent": sle.serial_and_batch_bundle},
				fields=["qty"]
			)
			for b in bundle_rolls:
				if b.qty > 0:
					in_rolls += 1
				elif b.qty < 0:
					out_rolls += 1

		# Track balance per item_code only
		key = sle.item_code

		# Initialize dictionary if not already
		if "roll_balance_map" not in locals():
			roll_balance_map = {}

		# Get previous balance for this item
		prev_rolls = roll_balance_map.get(key, 0)

		# Update balance
		bal_qty_rolls = prev_rolls + in_rolls - out_rolls

		# Store updated balance
		roll_balance_map[key] = bal_qty_rolls

		# Update the current SLE row
		sle.update({
			"in_rolls": in_rolls,
			"out_rolls": out_rolls,
			"bal_qty_rolls": bal_qty_rolls,
		})
		# --- end rolls ---
...
def get_columns(filters):
	...
	columns.extend(
		[
			...
			{
				"label": _("Balance Qty (Rolls)"),
				"fieldname": "bal_qty_rolls",
				"fieldtype": "Float",
				"width": 120,
			},
			{
